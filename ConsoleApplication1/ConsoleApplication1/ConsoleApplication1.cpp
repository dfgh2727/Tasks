...
void PlayerStatusRender(const char* const _AttName, const char* const _DefName)
{
    StatusRender(PlayerName, PlayerAtt, PlayerHp);
    printf_s("%s 가 %s를 공격해서 %d의 데미지를 입혔습니다.\n", _AttName, _DefName, PlayerAtt);
}


void MonsterStatusRender(const char* const _AttName, const char* const _DefName)
{
    StatusRender(MonsterName, MonsterAtt, MonsterHp);
    printf_s("%s 가 %s를 공격해서 %d의 데미지를 입혔습니다.\n", _AttName, _DefName, MonsterAtt);
}
...
int main()
{
    ...
    while (true)
    {
        ...
        PlayerStatusRender(PlayerName, MonsterName);
        MonsterStatusRender(MonsterName, PlayerName);
        Input = _getch();
